...
Runner.prototype = {
    ...
    if (setting in this.config && value != undefined) {
      ...
      switch (setting) {
        ...
        case 'SPEED_DROP_COEFFICIENT':
          this.cat.config[setting] = value;
          break;
        case 'INITIAL_JUMP_VELOCITY':
          this.cat.setJumpVelocity(value);
          break;
        case 'SPEED':
          this.setSpeed(value);
          break;
  ...
  update: function() {
    ...
    if (this.activated) {
      ...
      if (this.cat.jumpCount == 1 && !this.playingIntro) {
        this.playIntro();
      }
      ...
      var collision = hasObstacles &&
          checkForCollision(this.horizon.obstacles[0], this.cat);
  ...
  onKeyDown: function(e) {
    ...
    if (e.target != this.detailsButton) {
      if (!this.crashed && (Runner.keycodes.JUMP[e.keyCode] ||
        ...
        if (!this.cat.jumping && !this.cat.ducking) {
          this.playSound(this.soundFx.BUTTON_PRESS);
          this.cat.startJump(this.currentSpeed);
        }
      }
    ...
    if (this.activated && !this.crashed && Runner.keycodes.DUCK[e.keyCode]) {
      ...
      if (this.cat.jumping) {
        ...
        this.cat.setSpeedDrop();
      } else if (!this.cat.jumping && !this.cat.ducking) {
        // Duck.
        this.cat.setDuck(true);
      }
    }
  },
  ...
  onKeyUp: function(e) {
    ...
    if (this.isRunning() && isjumpKey) {
      ...
      this.cat.speedDrop = false;
      this.cat.setDuck(false);
    } else if (this.crashed) {
  ...
  gameOver: function() {
    ...
    this.cat.update(100, Cat.status.CRASHED);
...
Cat.collisionBoxes = {
  DUCKING: [new CollisionBox(1, 21, 58, 29)],
  RUNNING: [
    new CollisionBox(30, 0, 23, 18),
    new CollisionBox(1, 21, 41, 10),
    new CollisionBox(14, 40, 19, 9),
    new CollisionBox(1, 28, 40, 6),
    new CollisionBox(7, 34, 29, 5),
    new CollisionBox(12, 39, 20, 5),
    new CollisionBox(44, 4, 15, 22)
  ]
};
...
Cat.animFrames = {
  WAITING: { frames: [60, 0], msPerFrame: 1000 / 3 },
  RUNNING: { frames: [120, 180], msPerFrame: 1000 / 12 },
  CRASHED: { frames: [300], msPerFrame: 1000 / 60 },
  JUMPING: { frames: [0], msPerFrame: 1000 / 60 },
  DUCKING: { frames: [360, 419], msPerFrame: 1000 / 8 }
};
